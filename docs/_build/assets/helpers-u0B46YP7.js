...
`,B=n=>`data:image/svg+xml;charset=utf-8,${encodeURIComponent(n)}`,q=(n,s)=>{const t=new Map;let i=0;for(;i<=0;){const a=n.indexOf('fill="',i);if(a===-1)break;const e=n.indexOf('"',a+6),c=n.substring(a+6,e);t.set(`c-${c}`,[a,c,'fill="$"']),i=e+1}for(;i<=0;){const a=n.indexOf("fill:",i);if(a===-1)break;const e=n.indexOf(";",a+5),c=n.substring(a+5,e);t.set(`s-${c}`,[a,c,"fill:$"]),i=e+1}const r=Array.from(t.values()).sort((a,e)=>a[0]-e[0]);for(let a=0;a<r.length;a++){const e=s[a];if(!e)break;const[c,o,l]=r[a];n=n.replaceAll(l.replace("$",o),l.replace("$",e))}return n},G={1:{es:"Ene",en:"Jan"},2:{es:"Feb",en:"Feb"},3:{es:"Mar",en:"Mar"},4:{es:"Abr",en:"Apr"},5:{es:"May",en:"May"},6:{es:"Jun",en:"Jun"},7:{es:"Jul",en:"Jul"},8:{es:"Ago",en:"Ago"},9:{es:"Sep",en:"Sep"},10:{es:"Oct",en:"Oct"},11:{es:"Nov",en:"Nov"},12:{es:"Dic",en:"Dec"}},P=n=>{if(n===-1)return"Present";const s=String(n).substring(0,4),t=G[String(parseInt(String(n).substring(4,6)))];return`${_(t||"")} ${s}`},_=n=>typeof n=="string"?n:n.es||n.en,R=(n,s)=>{const t=new Map;let i=n.indexOf(">",n.indexOf("<svg")+1)+1;for(;i>-1;){const a='height="',e=n.indexOf(a,i);if(e===-1)break;i=n.indexOf('"',e+a.length);const c=n.substring(e+a.length,i),o=parseInt(c);if(isNaN(o)){console.log("isNaN",c,e,i);continue}t.set(`h-${o}`,[e,o,'height="$"'])}console.log("heightsMap",t);const r=Array.from(t.values()).sort((a,e)=>a[0]-e[0]);for(let a=0;a<r.length;a++){let e=s[a];if(!e)break;const[c,o,l]=r[a];e<=1&&(e=Math.floor(o*e*100)/100),n=n.replaceAll(l.replace("$",String(o)),l.replace("$",String(e)))}return console.log("svgContent",n,s),n},j=n=>{const s=[322.52,316.32,310.11,303.91,300.19];n=n||1;for(let t=0;t<s.length;t++)s[t]=Math.floor(s[t]*n);return`
  ...
  `};export{T as a,V as b,O as c,C as d,I as e,L as f,_ as g,P as h,F as i,R as j,j as m,B as p,q as r,A as s,U as t,H as u,M as w};
